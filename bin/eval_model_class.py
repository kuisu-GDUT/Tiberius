...
from gene_pred_hmm import class3_emission_matrix, GenePredHMMLayer, make_5_class_emission_kernel, \
    make_aggregation_matrix, make_15_class_emission_kernel
from learnMSA.msa_hmm.Initializers import ConstantInitializer
from transformers import AutoTokenizer, TFAutoModelForMaskedLM, TFEsmForMaskedLM
from tensorflow.keras.layers import (Conv1D, SimpleRNN, Conv1DTranspose, LSTM, GRU, Dense, Bidirectional, Dropout,
                                     Activation, Input, BatchNormalization, LSTM, Reshape, Embedding, Add,
                                     LayerNormalization,
                                     AveragePooling1D)
...
class PredictionGTF:
    ...
    def transformer_prediction(self, inp_ids, attention_mask, save=True):
        ...
        if save and self.temp_dir:
            if self.emb:
                np.savez(f'{self.temp_dir}/trans_predictions.npz', array1=trans_predictions[0],
                         array2=trans_predictions[1])
            else:
    ...
    def lstm_prediction(self, inp_chunks, clamsa_inp=None, trans_emb=None, save=True, batch_size=None):
        ...
        if save and self.temp_dir:
            if self.emb:
                np.savez(f'{self.temp_dir}/lstm_predictions.npz', array1=lstm_predictions[0],
                         array2=lstm_predictions[1])
            else:
    ...
    def hmm_predictions_filtered(self, inp_chunks, lstm_predictions, save=True, batch_size=None):
        ...
        if self.hmm_factor > 1:
            inp_chunks = inp_chunks.reshape(
                (inp_chunks.shape[0] * self.hmm_factor, inp_chunks.shape[1] // self.hmm_factor, -1))
            lstm_predictions[0] = lstm_predictions[0].reshape(
                (lstm_predictions[0].shape[0] * self.hmm_factor, lstm_predictions[0].shape[1] // self.hmm_factor, -1))
            lstm_predictions[1] = lstm_predictions[1].reshape(
                (lstm_predictions[1].shape[0] * self.hmm_factor, lstm_predictions[1].shape[1] // self.hmm_factor, -1))
    ...
    def get_predictions(self, inp_chunks, clamsa_inp=None, hmm_filter=False, save=True, encoding_layer_oracle=None,
                        batch_size=None):
    ...
    def create_gtf(self, y_label, coords, f_chunks, out_file='', clamsa_inp=None,
        ...
        for seq in ranges:
            new_tx = False
            phase = -1
            for tx in ranges[seq]:  # 一个转录本对应一个列表， 列表中的每个元素是一个区域
                tx_id += 1
                t_id = f'g{tx_id}.t1'
                g_id = f'g{tx_id}'
                phase = 0
