...
def add_hmm_layer(model, gene_pred_layer=None, dense_size=128, pool_size=9,
    ...
    if x.shape[-1] > output_size:
        if x.shape[-1] == 7:
            ...
            raise ValueError(
                f"Invalid combination of loaded output size ({x.shape[-1]}) and requested output size ({output_size}).")
...
def transformer_model(units=200, d_model=256, num_heads=4, filter_size=64, kernel_size=9,
    ...
    x = Dense(128, activation="relu", name="resize_for_longformer")(
        x)  # 768 is one of the standard sizes for Longformer
